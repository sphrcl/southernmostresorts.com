...
function youtube_duration ($id) {
        $file = file_get_contents("https://www.googleapis.com/youtube/v3/videos?part=contentDetails,statistics&id=$id&key=AIzaSyBYmYjr257oPZLWfsYDjv3-eo0YDaO2ZKA");
         $duration =json_decode($file, true);
        foreach ($duration['items'] as $vidTime) {
            $vTime= $vidTime['contentDetails']['duration'];
            $vTime = covtime($vTime);
            $parsed = date_parse($vTime);
            $seconds = $parsed['hour'] * 3600 + $parsed['minute'] * 60 + $parsed['second'];
        }
    return $seconds;
}
function covtime($vTime){
