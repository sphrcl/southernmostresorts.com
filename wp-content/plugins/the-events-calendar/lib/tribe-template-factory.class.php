...
if ( ! class_exists( 'Tribe_Template_Factory' ) ) {
	class Tribe_Template_Factory {
		...
		final public function title_tag( $title, $sep = null ) {
			...
			$titledesc = "Boutique Hotel in Key West - Key West Hotels - The Gates of Key West";
			return apply_filters( 'tribe_events_title_tag', apply_filters( 'tribe_events_add_title', $new_title, $title, $sep ), $title, $sep, $titledesc );
		}
		...
		protected function get_title( $title, $sep = null ) {
			return tribe_get_events_title( false ) . ' ' . $sep . ' ' . $title . 'Boutique Hotel in Key West - Key West Hotels - The Gates of Key West';
		}
