...
$meta_boxel_dining = array(
	...
	'fields' => array(		
		...
		array( 
			"name" => "Dining Menu PDF",
			"id" => $prefix."_dining_menu",
			"type" => "upload_file",
			"std" => ""
		),
		...
		array( 
			"name" => "Wedding PDF",
			"id" => $prefix."_wedding_menu",
			"type" => "upload_file",
			"std" => ""
		),

		array( 
			"name" => "Catering Factsheet PDF",
			"id" => $prefix."_catering_menu",
			"type" => "upload_file",
			"std" => ""
		),
...
add_action('admin_menu', 'dining_add_boxel');
// Add meta boxel
function dining_add_boxel() {
	global $meta_boxel_dining;
	foreach ( array( 'dining' ) as $page )
	add_meta_box($meta_boxel_dining['id'], $meta_boxel_dining['title'], 'dining_show_boxel', $page, $meta_boxel_dining['context'], 			$meta_boxel_dining['priority']);
}
// Callback function to show fields in meta boxel
function dining_show_boxel() {
	global $meta_boxel_dining, $post;
...
jQuery(document).ready(function() {
	...
	echo '<input type="hidden" name="dining_meta_boxel_dining_nonce" value="', wp_create_nonce(basename(__FILE__)), '" />';
	echo '<table class="form-table">';
	foreach ($meta_boxel_dining['fields'] as $field) {
...
add_action('save_post', 'dining_save_data');
// Save data from meta boxel
function dining_save_data($post_id) {
	global $meta_boxel_dining;	
	// verify nonce
	if (!wp_verify_nonce($_POST['dining_meta_boxel_dining_nonce'], basename(__FILE__))) {
		return $post_id;
	}
	// check autosave
