...
$activities_boxer = array(
	...
	'fields' => array(
 		...
 		,
 		array( 
              "name" => "Location Longitude of Activity",
	          "desc" => "EX: -87.913027",
	          "id" => $prefix."_long",
	          "type" => "text",
	          "std" => ""
...
add_action('admin_menu', 'activities_themer_add_boxer');
// Add meta boxer
function activities_themer_add_boxer() {
	global $activities_boxer;
	foreach ( array( 'activities' ) as $page )
	add_meta_box($activities_boxer['id'], $activities_boxer['title'], 'activities_themer_show_boxer', $page, $activities_boxer['context'], 			$activities_boxer['priority']);
}
// Callback function to show fields in meta boxer
function activities_themer_show_boxer() {
	global $activities_boxer, $post;
...
jQuery(document).ready(function() {
	...
	echo '<input type="hidden" name="activities_themer_activities_boxer_nonce" value="', wp_create_nonce(basename(__FILE__)), '" />';
	echo '<table class="form-table">';
	foreach ($activities_boxer['fields'] as $field) {
...
add_action('save_post', 'activities_themer_save_data');
// Save data from meta boxer
function activities_themer_save_data($post_id) {
	global $activities_boxer;	
	// verify nonce
	if (!wp_verify_nonce($_POST['activities_themer_activities_boxer_nonce'], basename(__FILE__))) {
		return $post_id;
	}
	// check autosave
