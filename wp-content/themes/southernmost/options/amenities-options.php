...
add_action('admin_menu', 'amenities_add_boxel');
// Add meta boxel
...
function amenities_show_boxel() {
	global $amenities_box, $post;
...
jQuery(document).ready(function() {
	...
	echo '<input type="hidden" name="amenities_meta_boxel_nonce" value="', wp_create_nonce(basename(__FILE__)), '" />';
	echo '<table class="form-table">';
	foreach ($amenities_box['fields'] as $field) {
...
function amenities_save_data($post_id) {
	global $amenities_box;	
	// verify nonce
	if (!wp_verify_nonce($_POST['amenities_meta_boxel_nonce'], basename(__FILE__))) {
		return $post_id;
	}
	// check autosave
